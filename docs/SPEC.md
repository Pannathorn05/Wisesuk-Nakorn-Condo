...
- ยืนยันอีเมล, เปลี่ยนอีเมล (ลืมรหัสผ่านย้ายเข้า scope แล้ว ดู AC-23)
- ระงับบัญชีสมาชิก
- ส่ง SMS ทุกกรณี และส่งอีเมลทุกกรณี **ยกเว้นอีเมลรีเซ็ตรหัสผ่านตาม AC-23**
  (แจ้งเตือนอื่นทั้งหมดยังเก็บในระบบให้ดึงไปแสดงเท่านั้น ไม่ส่งออกนอกระบบ)
- **Admin/Super Admin รีเซ็ตรหัสผ่านให้ผู้ใช้คนอื่น** — v1 ให้ผู้ใช้ทำเองผ่าน AC-23 เท่านั้น
...
AC-8: ห้องเดียวจองซ้อนไม่ได้ (กฎเหล็ก)
  ...
  - **URL ทุกเส้นใช้ id ที่มี prefix ของตาราง (bkg-001) ไม่ใช่ booking_code** — `GET /api/v1/bookings/PT-000001` → 400 (booking_code ใช้แสดงผลและค้นหาเท่านั้น)
...
AC-21: รหัสสถานะและรูปแบบ error สอดคล้องกันทั้งระบบ
  - **400 bad_request** ใช้เมื่อ: พาร์ส body ไม่ได้, ชนิดข้อมูลผิด, query param ผิดรูป (branch_id ไม่อยู่ในรูปแบบ brn-001 หรือใช้ prefix ของตารางอื่น, page ไม่ใช่ตัวเลข), ไฟล์อัปโหลดไม่ถูกต้อง
...
AC-23: ลืมรหัสผ่าน — ขอลิงก์รีเซ็ตทางอีเมลแล้วตั้งรหัสใหม่เองได้
  - POST forgot-password ด้วยอีเมลที่มีบัญชีอยู่จริง → **204** และมีอีเมล 1 ฉบับถูกส่งไปที่อีเมลนั้น
  - POST forgot-password ด้วยอีเมลที่ไม่มีในระบบ → **204 เหมือนกันทุกประการ** (status, body ว่าง, header)
    และ **ไม่มีอีเมลถูกส่งออกเลย** — ห้ามบอกใบ้ว่าอีเมลใดมีบัญชี
  - จับเวลา 2 กรณีข้างบนกรณีละ 50 ครั้ง เทียบค่า **median** → ต่างกันไม่เกิน 20%
    (ต้องส่งอีเมลแบบไม่บล็อก response และเผาเวลาด้วย dummy hash เมื่อไม่พบอีเมล เหมือน AC-2)
  - บัญชี is_active = false → 204 เช่นกัน แต่ไม่มีอีเมลถูกส่ง (บัญชีถูกปิดอยู่ รีเซ็ตแล้วก็ล็อกอินไม่ได้)
  - ลิงก์ในอีเมลชี้ไป `PUBLIC_APP_URL/reset-password?token=<token ดิบ>` — ค่า `PUBLIC_APP_URL`
    เป็นโดเมนหน้าเว็บ **คนละตัวกับ** `PUBLIC_BASE_URL` ที่เป็นโดเมน API สำหรับไฟล์รูป
  - ตาราง password_resets เก็บเฉพาะ sha256 hash ของ token ไม่มี token ดิบ (เหมือน refresh_tokens ใน AC-3)
  - POST reset-password ด้วย token ถูกต้อง + รหัสใหม่ถูกกติกา → 204 และล็อกอินด้วยรหัสใหม่ได้ทันที
  - รหัสใหม่ผิดกติกา / confirm_password ไม่ตรง → 422 code "validation_failed" + fields
  - token อายุเกิน **15 นาที** → 422 code "validation_failed" + fields.token
  - token ที่ใช้รีเซ็ตไปแล้ว ใช้ซ้ำอีกครั้ง → 422 + fields.token (ใช้ได้ครั้งเดียวเท่านั้น)
  - token ที่ถูกแทนที่เพราะขอใบใหม่ → 422 + fields.token
    (ขอ forgot-password ซ้ำ → ใบเก่าทุกใบของบัญชีนั้นใช้ไม่ได้ทันที เหลือใบล่าสุดใบเดียว)
  - สตริงที่ไม่ใช่ token เลย → 422 + fields.token **ข้อความเดียวกันกับ 3 กรณีข้างบนทุกตัวอักษร**
    (ไม่บอกว่าหมดอายุ ถูกใช้ไปแล้ว หรือไม่มีอยู่จริง)
  - ยิงพร้อมกัน 2 request ด้วย token ใบเดียวกัน → สำเร็จ 1 ใบเท่านั้น (บังคับที่ database เหมือน AC-3)
  - รีเซ็ตสำเร็จ → refresh token ทุกใบของบัญชีนั้นใช้ไม่ได้ทันที (เหมือนเปลี่ยนรหัสผ่านใน AC-3)
    **ยอมรับความเสี่ยง:** access token ใบเดิมยังใช้ได้จนหมดอายุ เหมือน AC-3
  - รีเซ็ตสำเร็จ → activity_logs มี 1 แถว action "user.reset_password" (ดู AC-13)
  - ขอ forgot-password ด้วยอีเมลเดียวกันเกิน **5 ครั้งใน 1 ชั่วโมง** → ครั้งที่ 6 ยังได้ 204
    แต่ไม่มีอีเมลถูกส่ง — บังคับด้วยการนับแถวใน password_resets ที่ database ไม่ใช่ในหน่วยความจำ
  - ตั้ง APP_ENV=production โดยไม่ตั้งค่า SMTP → server ไม่ start และ log บอกชื่อ env ที่ขาด
    (เหมือน JWT_SECRET ที่ตรวจตอน boot อยู่แล้ว — ห้ามให้ production ขึ้นมาโดยส่งอีเมลไม่ได้เงียบ ๆ)
  - ส่งอีเมลไม่สำเร็จ (SMTP ล่ม) → ผู้ใช้ยังได้ 204 ตามเดิม และ log ระดับ error ฝั่งเซิร์ฟเวอร์
...
POST /api/v1/auth/forgot-password {email}     → 204 เสมอ (ไม่บอกใบ้ว่าอีเมลมีบัญชีหรือไม่)
POST /api/v1/auth/reset-password {token,new_password,confirm_password}
                                              → 204 (เพิกถอน refresh token ทุกใบ) | 400 | 422
