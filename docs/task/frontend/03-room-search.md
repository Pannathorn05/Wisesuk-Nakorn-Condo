...
- **`stay_type` ผิดค่า/ไม่รู้จัก → 400** (`"stay_type ต้องเป็น daily หรือ monthly"`), **`branch_id` ที่ไม่อยู่ในรูปแบบ brn-001 → 400** เช็คแล้วตรงกับ error envelope เดิมที่ `api/client.js` แปลงให้อยู่แล้ว ไม่ต้องเขียน error handling เพิ่ม
